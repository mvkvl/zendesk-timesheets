...
//    }
    ...
    public List<Ticket> searchTickets(String query) {
        for (int i = 0; i < maxRequestAttempts; i++) {
            try {
                return StreamSupport
                    .stream(zendesk.getSearchResults(Ticket.class, query).spliterator(), true)
                    .collect(Collectors.toList());
            } catch (ZendeskResponseRateLimitException rateLimit) {
                apiRateLimitWait(rateLimit.getRetryAfter());
            } catch (ZendeskResponseException e) {
                log.warn("zendesk exception occurred requesting search results: {} {}", e.getStatusCode(), e.getStatusText());
                if (log.isTraceEnabled())
                    log.warn("{}", e.getBody());
            } catch (Exception e) {
                log.warn("error requesting search results from zendesk: {}", e.getMessage());
                if (log.isTraceEnabled())
                    e.printStackTrace();
            }
        }
        log.info("maximum request attempts reached, no search results received from zendesk");
        return Collections.emptyList();
    }
    public List<Ticket> getTickets() {
        for (int i = 0; i < maxRequestAttempts; i++) {
            try {
                return StreamSupport
                    .stream(zendesk.getTickets().spliterator(), true)
                    .collect(Collectors.toList());
            } catch (ZendeskResponseRateLimitException rateLimit) {
                apiRateLimitWait(rateLimit.getRetryAfter());
            } catch (ZendeskResponseException e) {
                log.warn("zendesk exception occurred requesting tickets: {} {}", e.getStatusCode(), e.getStatusText());
                if (log.isTraceEnabled())
                    log.warn("{}", e.getBody());
            } catch (Exception e) {
                log.warn("error requesting tickets from zendesk: {}", e.getMessage());
                if (log.isTraceEnabled())
                    e.printStackTrace();
            }
        }
        log.info("maximum request attempts reached, no tickets received from zendesk");
        return Collections.emptyList();
    }
    public List<Audit> getAudits(Ticket ticket) {
        for (int i = 0; i < maxRequestAttempts; i++) {
            try {
                return StreamSupport
                        .stream(zendesk.getTicketAudits(ticket).spliterator(), true)
                        .collect(Collectors.toList());
            } catch (ZendeskResponseRateLimitException rateLimit) {
                apiRateLimitWait(rateLimit.getRetryAfter());
            } catch (ZendeskResponseException e) {
                log.warn("zendesk exception occurred requesting ticket audits: {} {}", e.getStatusCode(), e.getStatusText());
                if (log.isTraceEnabled())
                    log.warn("{}", e.getBody());
            } catch (Exception e) {
                log.warn("error requesting ticket audits from zendesk: {}", e.getMessage());
                if (log.isTraceEnabled())
                    e.printStackTrace();
            }
        }
        log.info("maximum request attempts reached, no ticket audits received from zendesk");
        return Collections.emptyList();
    }
    public Optional<User> getUser(Long userId) {
        for (int i = 0; i < maxRequestAttempts; i++) {
            try {
                return Optional.ofNullable(zendesk.getUser(userId));
            } catch (ZendeskResponseRateLimitException rateLimit) {
                apiRateLimitWait(rateLimit.getRetryAfter());
            } catch (ZendeskResponseException e) {
                log.warn("zendesk exception occurred requesting user: {} {}", e.getStatusCode(), e.getStatusText());
                if (log.isTraceEnabled())
                    log.warn("{}", e.getBody());
            } catch (Exception e) {
                log.warn("error requesting user from zendesk: {}", e.getMessage());
                if (log.isTraceEnabled())
                    e.printStackTrace();
            }
        }
